    ...
    for (i = 0; i < MAP_ROW; i++)
    {
        for (j = 0; j < MAP_COL; j++)
        {
            if (map[i][j] != BLANK_STATE)
            {
                ...
                // 绘制图案
                cell_rect = QRect(QPoint(FRAME_WIDTH*j+1, FRAME_HEIGHT*i), QSize(IMG_WIDTH, IMG_HEIGHT));
                img_rect = QRect(QPoint(0, map[i][j]*(IMG_HEIGHT+6)), QSize(IMG_WIDTH, IMG_HEIGHT));
